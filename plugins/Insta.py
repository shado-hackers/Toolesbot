...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="Thank you for using - @urlx_bot")
        else:
            dump_file=await message.reply_video(url,caption="Thank you for using - @urlx_bot")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   ...
                   dump_file=await message.reply_video(downfile,caption="Thank you for using - @urlx_bot") 
            elif "/p/" in url:
                  meta_tag = requests.post("https://saveig.app/api/ajaxSearch", data={"q": link, "t": "media", "lang": "en"}, headers=headers)
                  if meta_tag.ok:
                     res=meta_tag.json()
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="Thank you for using - @urlx_bot")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="Thank you for using - @urlx_bot")
                  except:
          ...
          #     await message.reply(tracemsg)
            ##optinal 
            await message.reply(f"400: Sorry, Unable To Find It  try another or report it  to @shado_hackers or support chat @urlx_bot 🤖  ")
